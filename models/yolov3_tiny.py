...
#        self.backbone = build_backbone(model_name='darknet_tiny', pretrained=trainable)
        self.backbone = build_backbone(model_name='darknet_tiny')        
        # s = 32
        ...
        # s = 16
        self.conv_set_1 = Conv(448, 256, k=3, p=1)
    ...
    def init_yolo(self):  
        ...
        # init obj&cls pred
        for pred in [self.pred_1, self.pred_2]:
    ...
    def inference(self, x):
        ...
        c2, c4, c5 = feats['layer0'], feats['layer2'], feats['layer3']
        ...
        p4 = torch.cat([c2, c4, p5_up], dim=1)
    ...
    def forward(self, x, target=None):
        if not self.trainable:
            ...
            # backbone
            feats = self.backbone(x)
            c2a, c4, c5 = feats['layer0'], feats['layer2'], feats['layer3']

            # FPN
            p5 = self.conv_set_2(c5)
            p5_up = F.interpolate(self.conv_1x1_2(p5), scale_factor=2.0, mode='bilinear', align_corners=True)

            p4 = torch.cat([c2a, c4, p5_up], dim=1)
            p4 = self.conv_set_1(p4)
